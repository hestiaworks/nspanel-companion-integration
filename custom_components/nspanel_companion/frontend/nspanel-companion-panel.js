...
class NSPanelCompanionPanel extends HTMLElement {
  ...
  widgetTemplate(type) {
    ...
    if (type === "entity_button") return { type, icon: "auto", show_timer: true, timer_presets: [5, 15, 30, 60], card_tap: false };
  ...
  pagePreview(page, miniature = false) {
    ...
    const widgetMarkup = widgets.length ? widgets.map((widget) => {
      ...
      if (widget.type === "thermostat") {
        ...
        return `<div class="preview-climate dual"><div class="climate-ring"><small>CURRENT</small><strong>${escapeHtml(actual)}°</strong><div class="ring-target heat">${escapeHtml(heat)}°</div><div class="ring-target cool">${escapeHtml(cool)}°</div></div><div class="target-picker"><button class="selected" type="button"><small>HEAT BELOW</small><b>${escapeHtml(heat)}°</b></button><button type="button"><small>COOL ABOVE</small><b>${escapeHtml(cool)}°</b></button></div><div class="target-stepper"><button type="button">−</button><span>Adjust heat target</span><button type="button">＋</button></div><div class="preview-modes"><b>Heat</b><b>Cool</b><b class="active">Auto</b><b>Fan</b><b>Dry</b><b>Off</b></div></div>`;
      }
      ...
      return `<div class="preview-tile control revised ${widget.card_tap ? "whole-card" : ""}"><div class="control-head"><i>${glyph}</i><button type="button" aria-label="Toggle"><span></span></button></div><b>${escapeHtml(name)}</b><small>${escapeHtml(automatic === "power" ? "Switch" : automatic)}</small><strong>${escapeHtml(entity?.state || "Off")}</strong>${widget.show_timer !== false ? `<div class="control-timer"><em>Timer</em>${presets.map((value) => `<span>${value}m</span>`).join("")}</div>` : ""}</div>`;
...
const STYLES = `
  ...
  *{box-sizing:border-box}main{max-width:1180px;margin:auto;padding:32px}header{display:flex;justify-content:space-between;align-items:start;margin-bottom:24px}h1{font-size:32px;margin:4px 0}h2{margin:0 0 6px;font-size:21px}h3{margin:0;font-size:17px}p{color:var(--secondary-text-color,#70746f);margin:4px 0;line-height:1.45}.eyebrow{text-transform:uppercase;letter-spacing:.14em;font-weight:800;font-size:11px;color:#f36d21}
  ...
  .preview-tile.control.revised{padding:8%;justify-content:flex-start}.control-head{display:flex;align-items:center;justify-content:space-between}.preview-tile.control.revised .control-head>i{font-size:7cqw}.control-head button{position:relative;width:28%;aspect-ratio:1.65;padding:0;border:0;border-radius:999px;background:#f36d21}.control-head button span{position:absolute;right:8%;top:14%;width:43%;aspect-ratio:1;border-radius:50%;background:#fff}.preview-tile.control.revised>b{margin-top:8%;font-size:4.4cqw;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.preview-tile.control.revised>small{font-size:3.2cqw;opacity:.68;text-transform:capitalize}.preview-tile.control.revised>strong{margin-top:auto;font-size:8cqw}.control-timer{display:flex;align-items:center;gap:3%;margin-top:6%;padding-top:5%;border-top:1px solid rgba(127,127,127,.25)}.control-timer em{position:static;font-style:normal;font-size:3cqw;padding:0;background:none}.control-timer span{position:static;width:auto;aspect-ratio:auto;padding:3% 5%;border-radius:7px;background:rgba(127,127,127,.13);font-size:2.7cqw}
`;
