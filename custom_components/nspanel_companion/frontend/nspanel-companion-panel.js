...
class NSPanelCompanionPanel extends HTMLElement {
  ...
  widgetTemplate(type) {
    ...
    if (type === "entity_button") return { type, icon: "auto", show_timer: true, card_tap: false };
    return { type };
  }
  ...
  widgetEditor(page, widget, index) {
    ...
    if (widget.type === "entity_button") configuration = `<label>Control entity<select ${field("entity_id")} required>${this.entityOptions(["light", "fan", "switch", "input_boolean", "cover"], entity, "Select light, fan, switch, or cover")}</select><small>The panel automatically uses the correct native control for this entity's capabilities.</small></label><div class="widget-fields control-options"><label>Icon<select ${field("icon")}><option value="auto" ${(widget.icon || "auto") === "auto" ? "selected" : ""}>Auto from entity</option><option value="light" ${widget.icon === "light" ? "selected" : ""}>Light</option><option value="fan" ${widget.icon === "fan" ? "selected" : ""}>Fan</option><option value="power" ${widget.icon === "power" ? "selected" : ""}>Power</option><option value="cover" ${widget.icon === "cover" ? "selected" : ""}>Curtains / cover</option><option value="plug" ${widget.icon === "plug" ? "selected" : ""}>Plug</option></select></label><div class="control-checks"><label class="check"><input type="checkbox" ${field("show_timer")} ${widget.show_timer !== false ? "checked" : ""}> Show timer</label><label class="check"><input type="checkbox" ${field("card_tap")} ${widget.card_tap === true ? "checked" : ""}> Use whole card as button</label></div></div>`;
  ...
  pagePreview(page, miniature = false) {
    ...
    const widgetMarkup = widgets.length ? widgets.map((widget) => {
      ...
      const icons = { auto: entity?.entity_id?.startsWith("fan.") ? "⌁" : entity?.entity_id?.startsWith("cover.") ? "▥" : "✦", light: "✦", fan: "⌁", power: "⏻", cover: "▥", plug: "⌑" };
      return `<div class="preview-tile control ${widget.card_tap ? "whole-card" : ""}"><i>${icons[widget.icon || "auto"] || "✦"}</i><small>${escapeHtml(name)}</small><strong>${escapeHtml(entity?.state || "Off")}</strong>${widget.show_timer !== false ? "<em>Tm</em>" : ""}<span></span></div>`;
